...
func (app *NetAssistantApp) updateSendCount(count int) {
	app.sendCount += count
	app.labelSendCount.SetText("Send Count:" + strconv.Itoa(app.sendCount))
}
...
func (app *NetAssistantApp) onBtnCleanCount() {
	app.receCount = 0
	app.sendCount = 0
	app.labelReceveCount.SetText("Recv Count:0")
	app.labelSendCount.SetText("Send Count:0")
}
...
func (app *NetAssistantApp) disconnect(serverType int) error {
	...
	if serverType == 3 {
		app.labelLocalAddr.SetLabel("Local IP")
		app.labelLocalPort.SetLabel("Local Port")
	...
	app.updateStatus("waiting connection")
...
func (app *NetAssistantApp) doActivate(application *gtk.Application) {
	...
	app.appWindow.SetTitle("Network Assistant")
	...
	frame, _ := gtk.FrameNew("Network Settings")
	...
	app.cbDisplayDate, _ = gtk.CheckButtonNewWithLabel("Show time and wrap")
	app.cbHexDisplay, _ = gtk.CheckButtonNewWithLabel("Hex display")
	app.cbPauseDisplay, _ = gtk.CheckButtonNewWithLabel("Pause display")
	...
	// Data Received
	titleDataReceiveArea, _ := gtk.LabelNew("Data Received")
	...
	app.labelLocalAddr, _ = gtk.LabelNew("Local IP")
	app.entryCurAddr, _ = gtk.EntryNew()
	app.entryCurAddr.SetEditable(false)
	app.labelLocalPort, _ = gtk.LabelNew("Local Port")
